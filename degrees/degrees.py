...
def shortest_path(source, target):
    ...
    # Make ready the frontier
    start = Node(state=source, parent=None, action=None)
    frontier = QueueFrontier()
    frontier.add(start)

    # For explored actors empty set()
    explored = set()

    # Create a loop for cheking the solution
    while True:
        # If nothing left in frontier, then no path
        if frontier.empty():
            return None
        # Choose the node from frontier
        node = frontier.remove()
        # Cheking
        #print(f"looking {node.state}")
        num_explored += 1
...
def person_id_for_name(name):
    ...
    resolving ambiguities as needed. retrieves the id for any person
    (and handles prompting the user to clarify,
    in the event that multiple people have the same name).
    """
